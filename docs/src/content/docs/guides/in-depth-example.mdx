...
For example, if an user has these two events in the log:
...
1. **Build initial state.** Inviting an user starts a brand-new lifecycle, so we mint a fresh `ulid()` (a sortable, URL-safe id) and start with an empty `UserState`. There is nothing to load from the event log yet.
...
The core function lives in <a href={EXAMPLES_URL + "/eventsourcing-demo/src/write/iam/users/core/commands/inviteUser.command.ts#L53-L87"}>`src/write/iam/users/core/commands/inviteUser.command.ts`</a>. It is the entire business definition of "what does it mean to invite an user".
