...
title: CRUD+
description: >-
  Low-level traced MongoDB CRUD helpers with EventFabric exceptions — use directly or
  via MongoDBRepository.
---
import InDepthExampleCard from '../../../components/InDepthExampleCard.astro';

The MongoDB package provides low-level CRUD functions for direct database operations. These functions are fully instrumented with OpenTelemetry tracing and metrics, and handle errors using EventFabric exceptions.
...
For standard CRUD operations, prefer using the [Repository](/mongodb/repository) class.
...
All functions use `handleMongoError` internally to convert MongoDB errors to EventFabric exceptions. See [handleMongoError](/mongodb/handle-mongo-error) for details on error mapping.
