...
title: Read Events
description: >-
  readEvents streams events for a subject from EventSourcingDB; use event
  mapping to restore Nimbus event shapes.
---
import InDepthExampleCard from '../../../components/InDepthExampleCard.astro';

# Read Events

The `readEvents` function reads events from EventSourcingDB for a given subject. It returns an async generator that yields raw EventSourcingDB events, which can be converted to Nimbus events using the [event mapping](/eventsourcingdb/event-mapping) utilities.
