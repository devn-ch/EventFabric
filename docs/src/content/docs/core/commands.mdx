...
```typescript
import { createCommand } from "@eventfabric-cqrs/core";
import { InviteUserCommand } from "./inviteUser.command.ts";

const commandForJane = createCommand<InviteUserCommand>({
    type: "ch.devn.eventfabric.invite-user",
    source: "eventfabric.devn.ch",
    data: {
